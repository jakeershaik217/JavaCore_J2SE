	...
	public void BYOExtExterior(XSSFWorkbook wb) {
		...
		try {
			...
			if(BYOExtractorUtils.verticalTable[0]){
				//rowCount++;
				//exteriorSheet.addMergedRegion(new CellRangeAddress(rowCount-1, rowCount-1, 0, 5));
				//cell = exteriorSheet.createRow(rowCount).createCell(0);
				//System.out.println(rowCount);
			...
			if(BYOExtractorUtils.verticalTable[0]){
				...
				while(l<25){
					...
					if(accTblContent != null){
						if(!accTblContent.id().contains("likes-and-labels-container")){
